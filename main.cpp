...
__fastcall TForm1::TForm1(TComponent *Owner) : TForm(Owner) {
  ...
  RecentFile =
      TPath::Combine(ExtractFilePath(Application->ExeName), "recent.ini");
...
void __fastcall TForm1::ScrollBarChange(TObject *Sender) {
  ...
  if (mnuViewSpreadViewNone->Checked) { // not spread view
    ...
    fn_LoadImage(
        Gdv0,
        (TFI *)flst->Items[ScrollBar->Position - 1 +
                           (int)(mnuViewSpreadViewRight->Checked ? 1 : 0)]);
  }
}
...
// ---------------------------------------------------------------------------
// [File]-[Exit]
void __fastcall TForm1::mnuFileExitClick(TObject *Sender) { exit(0); }
...
// ---------------------------------------------------------------------------
// [Edit]-[Image Copy]
void __fastcall TForm1::mnuEditCopyClick(TObject *Sender) { fn_ImageCopy(); }

// ---------------------------------------------------------------------------
// [Edit]-[Path Copy]
void __fastcall TForm1::mnuEditPathClick(TObject *Sender) { fn_PathCopy(); }
...
// ---------------------------------------------------------------------------
// [View]-[Actual Siza]
void __fastcall TForm1::mnuViewActualClick(TObject *Sender) { fn_ZoomMode(1); }

// ---------------------------------------------------------------------------
// [View]-[Bestfit]
void __fastcall TForm1::mnuViewBestfitClick(TObject *Sender) { fn_ZoomMode(2); }
...
void __fastcall TForm1::mnuViewSpreadClick(TObject *Sender) { fn_ZoomMode(6); }
...
// ---------------------------------------------------------------------------
//
void __fastcall TForm1::mnuPlayPrevClick(TObject *Sender) { fn_Prev(); }

// ---------------------------------------------------------------------------
//
void __fastcall TForm1::mnuPlayNextClick(TObject *Sender) { fn_Next(); }
...
// ---------------------------------------------------------------------------
// [Tools]-[Options]
void __fastcall TForm1::mnuToolsOptionClick(TObject *Sender) { fn_Options(); }
...
// ---------------------------------------------------------------------------
// [ToolBar]-[Prev]
void __fastcall TForm1::tbtnPrevClick(TObject *Sender) { fn_Prev(); }

// ---------------------------------------------------------------------------
// [ToolBar]-[Next]
void __fastcall TForm1::tbtnNextClick(TObject *Sender) { fn_Next(); }

// ---------------------------------------------------------------------------
// [ToolBar]-[Bestfit]
void __fastcall TForm1::tbtnBestfitClick(TObject *Sender) { fn_ZoomMode(2); }

// ---------------------------------------------------------------------------
// [ToolBar]-[In window]
void __fastcall TForm1::tbtnInWindowClick(TObject *Sender) { fn_ZoomMode(99); }

// ---------------------------------------------------------------------------
// [ToolBar]-[Actual size]
void __fastcall TForm1::tbtnActualClick(TObject *Sender) { fn_ZoomMode(1); }

// ---------------------------------------------------------------------------
// [ToolBar]-[Spread to window]
void __fastcall TForm1::tbtnSpreadClick(TObject *Sender) { fn_ZoomMode(6); }
...
// ---------------------------------------------------------------------------
// [ToolBar]-[Slide Show]
void __fastcall TForm1::tbtnSlideShowClick(TObject *Sender) { fn_SlideShow(); }

// ---------------------------------------------------------------------------
// [ToolBar]-[Zoom in]
void __fastcall TForm1::tbtnZoomInClick(TObject *Sender) { fn_ZoomIn(); }

// ---------------------------------------------------------------------------
// [ToolBar]-[Zoon out]
void __fastcall TForm1::tbtnZoomOutClick(TObject *Sender) { fn_ZoomOut(); }
...
HBITMAP __fastcall TForm1::SPI_LoadImage(String fileName) {
  /* �Ή��v���O�C���̌��� */
  for (int i = 0; i < hSPI->Count;
       i++) { // �v���O�C���֐��̎擾
    SPI_ISSUPPORTED spi_issupported = (SPI_ISSUPPORTED)GetProcAddress(
        (HMODULE)hSPI->Items[i], SPIPROC_ISSUPPORTED);
    SPI_GETPICTURE spi_getpicture = (SPI_GETPICTURE)GetProcAddress(
        (HMODULE)hSPI->Items[i], SPIPROC_GETPICTURE);
    ...
    // �擾������񂩂�BITMAP�n���h���𐶐�����
    HDC dc = GetDC(0);
    HBITMAP bitmap = CreateDIBitmap(dc, &bmpinfo->bmiHeader, CBM_INIT, bmbits,
...
// ---------------------------------------------------------------------------
// Mouse Gestures
void __fastcall TForm1::Gdv0MouseDownControl(TObject *Sender, short *Button,
...
// ---------------------------------------------------------------------------
//
void __fastcall TForm1::Gdv0MouseMoveControl(TObject *Sender, short *Button,
                                             short *shift, float *X, float *Y) {
  //
}
...
// ---------------------------------------------------------------------------
//
void __fastcall TForm1::Gdv0MouseUpControl(TObject *Sender, short *Button,
                                           short *shift, float *X, float *Y) {
...
void __fastcall TForm1::ExecAction(String value) {
  ...
  // fn_FullScreen(false);
    ...
    fn_FileOpenDialog(fdoAllowMultiSelect | fdoPathMustExist |
                      fdoFileMustExist);
...
// ---------------------------------------------------------------------------
//
void __fastcall TForm1::TimerTimer(TObject *Sender) { fn_Next(); }
